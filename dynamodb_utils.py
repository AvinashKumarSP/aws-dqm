...
def put_rule(source, entity, cde, kde, rules_run_on, single_column_validation_rule, dynamodb=None):
    if not dynamodb:
        dynamodb = boto3.resource('dynamodb', endpoint_url="http://localhost:8000")
    ...
    response = table.put_item(
        Item={
            ...
            'cde': cde,
            'kde': kde,
            'single_column_validation_rule': single_column_validation_rule
        }
    )
    return response
...
def update_rule(source, entity, single_column_validation_rule, dynamodb=None):
    if not dynamodb:
        dynamodb = boto3.resource('dynamodb', endpoint_url="http://localhost:8000")
    ...
    response = table.update_item(
        ...
        UpdateExpression="set rules_run_on=:r, single_column_validation_rule=:a",
        ExpressionAttributeValues={
            ':r': ["cde", "kde"],
            ':a': single_column_validation_rule
        },
